...
const DEFAULT_WORKER_PATH = '.open-next/worker.js'
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (entry === '--worker') {
      workerPath = readFlagValue(argv, index, '--worker')
      index += 1
      continue
    }
  ...
  return { workerPath, maxGzipBytes }
}

function verifyCloudflareWorkerSize(args) {
  const fullPath = resolve(args.workerPath)
  if (!existsSync(fullPath)) {
    throw new WorkerMissingError(args.workerPath)
  }

  const worker = readFileSync(fullPath)
  const gzipBytes = gzipSync(worker, { level: 9 }).byteLength
  if (gzipBytes > args.maxGzipBytes) {
    throw new WorkerTooLargeError(args.workerPath, worker.byteLength, gzipBytes, args.maxGzipBytes)
  }
  ...
  return {
    ...
    summary: `Cloudflare Worker gzip 体积 ${gzipBytes} bytes，未超过限制 ${args.maxGzipBytes} bytes。`
  }
}
