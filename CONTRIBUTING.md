...
### 正式保存后的部署一致性原则
- 本项目默认认为：**本地正式保存后，本地页面能看到的正式内容，提交并部署后线上也应看到一致结果**。
- `src/config/*.json`、`public/**` 等会进入仓库并参与构建的正式源/正式产物，是部署后的权威数据来源。
- `localStorage`、浏览器缓存、草稿文件、备份文件和本地运行时临时数据只能作为编辑体验缓存或恢复辅助，不能长期静默覆盖已部署的正式源。
- 若页面需要优先读取本地缓存，必须提供版本、同步、失效或刷新机制，确保旧缓存不会遮蔽新部署内容；例如自定义组件、布局、主题色等首页配置，线上加载时不能让旧 `localStorage` 永久覆盖新提交的配置。
- 排查“本地可见但线上不可见”问题时，优先检查对应内容是否写入可部署文件、是否已提交/推送，以及线上浏览器是否被旧缓存覆盖。
