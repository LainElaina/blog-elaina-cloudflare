...
test('pictures upload dialog revokes rejected and closed file preview urls', async () => {
	...
	assert.match(source, /setImages\(current => \[\.\.\.current, \.\.\.nextImages\]\)/)
	assert.doesNotMatch(source, /revokeImagePreviews\(images\)\n\s*setImages\(nextImages\)/)
