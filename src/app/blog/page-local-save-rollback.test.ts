...
test('blog local save rolls back written artifacts when deleting removed article directories fails', async () => {
	...
	const rollbackThrowIndex = source.indexOf("throw new Error(`${originalMessage}；本地博客保存回滚失败：${rollbackMessage}`)", rollbackIndex)
	const throwOriginalIndex = source.indexOf('throw error', rollbackIndex)

	assert.ok(rollbackIndex > catchIndex)
	assert.ok(rollbackThrowIndex > rollbackIndex)
	assert.ok(throwOriginalIndex > rollbackThrowIndex)
	assert.doesNotMatch(source.slice(rollbackIndex, throwOriginalIndex), /console\.warn\('本地博客保存回滚失败:', rollbackError\)/)
