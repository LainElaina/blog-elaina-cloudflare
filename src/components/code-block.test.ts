...
test('code block component exposes long-line reading controls', () => {
	...
	assert.match(componentSource, /const \[wrapped, setWrapped\] = useState\(true\)/)
...
test('article styles define wrapped and fullscreen code block modes', () => {
	...
	assert.match(articleStyles, /\.code-block-fullscreen-overlay \.code-block-actions\s*\{[\s\S]*position:\s*absolute/)
	assert.match(articleStyles, /\.code-block-fullscreen-overlay \.code-block-actions\s*\{[\s\S]*top:\s*0\.75rem/)
	assert.match(articleStyles, /\.code-block-fullscreen-overlay \.code-block-actions\s*\{[\s\S]*right:\s*0\.75rem/)
	assert.match(articleStyles, /\.code-block-fullscreen-overlay \.code-block-wrapper--fullscreen > pre\s*\{[\s\S]*overflow:\s*auto/)
	assert.match(articleStyles, /\.code-block-fullscreen-overlay \.code-block-wrapper--fullscreen > pre\s*\{[\s\S]*padding:\s*3\.25rem 1\.25rem 1\.25rem/)
	assert.doesNotMatch(articleStyles, /code-block-action-btn--fullscreen-close/)
	assert.doesNotMatch(articleStyles, /color:\s*#fff/)
})
